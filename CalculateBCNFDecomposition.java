...
public class CalculateBCNFDecomposition extends CalculateDecomposition {
	...
	private List<Relation> decomposeBCNFHelper(final Relation r) {
		...
		for (FunctionalDependency f : r.getNormalFormsResults().getBCNFViolatingFDs()) {
			...
			List<FunctionalDependency> r1FDs = RDTUtils.fetchFDsOfDecomposedR(RDTUtils.getSingleAttributeMinimalCoverList(r.getDerivedFDs(), r), leftSideClosure.getClosure());
			...
			List<FunctionalDependency> r2FDs = RDTUtils.fetchFDsOfDecomposedR(RDTUtils.getSingleAttributeMinimalCoverList(r.getDerivedFDs(), r), r2Attributes);
